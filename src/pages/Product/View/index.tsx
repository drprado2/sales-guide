import React, { useEffect, useRef } from 'react';
import './styles.scss';
import { useDispatch, useSelector } from 'react-redux';
import { Card } from 'primereact/card';
import { useParams, useHistory, useLocation } from 'react-router-dom';
import { Skeleton } from 'primereact/skeleton';
import { ProgressSpinner } from 'primereact/progressspinner';
import { Divider } from 'primereact/divider';
import { format } from 'date-fns';
import { generatePath } from 'react-router';
import { Zoom } from '@material-ui/core';
import { Helmet } from 'react-helmet';
import { Button } from 'primereact/button';
import { confirmDialog } from 'primereact/confirmdialog';
import { Toast } from 'primereact/toast';
import ImageGallery from 'react-image-gallery';
import { TabPanel, TabView } from 'primereact/tabview';
import { allRoutes } from '../../../store/modules/auth/slice';
import { pushBreadcrumb, setCurrentPage } from '../../../store/modules/template/slice';
import {
  deleteRequest, getById, resetUpdateForm, resetViewData,
} from '../../../store/modules/products/slice';
import { StoreState } from '../../../store';
import ProductProvesSentView, { Location } from '../components/ProductProvesSentView';
import ProductTreinamentView, { Treinament } from '../components/ProductTreinamentView';

const mockLocations : Array<Location> = [
  {
    latitude: -23.42054773159478,
    longitude: -51.93330410257243,
    title: 'Genko',
    date: new Date(2021, 2, 12, 8, 14),
    description: 'Gondola montada para loja Genko, utilizei a exposição conforme explicado no treinamento',
    seller: { name: 'Adriano Oliveira', image: 'https://www.headshotsprague.com/wp-content/uploads/2019/07/Headshots_Prague-emotional-portrait-of-a-smiling-entrepreneur-1.jpg' },
    proveImages: [
      'https://static.wixstatic.com/media/cc9c78_e95524fcffb144febf2a34cfeefce723~mv2.jpg/v1/fill/w_1600,h_1067,al_c,q_90/cc9c78_e95524fcffb144febf2a34cfeefce723~mv2.webp',
      'https://thumbs.dreamstime.com/z/parte-dianteira-da-loja-de-asics-35797287.jpg',
      'https://i.pinimg.com/originals/c8/f8/a2/c8f8a294b3bd170b177fa3778433cd2c.jpg',
    ],
  },
  {
    latitude: -23.438938861268994,
    longitude: -51.93313860433504,
    title: 'Lojas Renner',
    date: new Date(2021, 2, 2, 14, 36),
    description: 'Gondola montada para Lojas Renner, eles ficaram muito felizes',
    seller: { name: 'João Pedro', image: 'https://ak.picdn.net/shutterstock/videos/19523383/thumb/3.jpg' },
    proveImages: [
      'https://static.wixstatic.com/media/cc9c78_e95524fcffb144febf2a34cfeefce723~mv2.jpg/v1/fill/w_1600,h_1067,al_c,q_90/cc9c78_e95524fcffb144febf2a34cfeefce723~mv2.webp',
      'https://thumbs.dreamstime.com/z/parte-dianteira-da-loja-de-asics-35797287.jpg',
      'https://i.pinimg.com/originals/c8/f8/a2/c8f8a294b3bd170b177fa3778433cd2c.jpg',
    ],
  },
  {
    latitude: -23.42053645199696,
    longitude: -51.97102593132208,
    title: 'Casas Bahia',
    date: new Date(2021, 1, 22, 9, 52),
    description: 'Gondola montada para Casas Bahia, iniciando as vendas a todo vapor',
    seller: { name: 'Paula Silva', image: 'https://qodebrisbane.com/wp-content/uploads/2019/07/This-is-not-a-person-10-1.jpeg' },
    proveImages: [
      'https://static.wixstatic.com/media/cc9c78_e95524fcffb144febf2a34cfeefce723~mv2.jpg/v1/fill/w_1600,h_1067,al_c,q_90/cc9c78_e95524fcffb144febf2a34cfeefce723~mv2.webp',
      'https://thumbs.dreamstime.com/z/parte-dianteira-da-loja-de-asics-35797287.jpg',
      'https://i.pinimg.com/originals/c8/f8/a2/c8f8a294b3bd170b177fa3778433cd2c.jpg',
    ],
  },
];

const mockTreinamentDones : Array<Treinament> = [
  {
    sellerName: 'Julio Cesar',
    sellerImage: 'https://www.headshotsprague.com/wp-content/uploads/2019/07/Headshots_Prague-emotional-portrait-of-a-smiling-entrepreneur-1.jpg',
    date: new Date(2021, 1, 22, 9, 52),
    hitPercentage: 88.6,
    passed: true,
  },
  {
    sellerName: 'Rael Augusto',
    sellerImage: 'https://ak.picdn.net/shutterstock/videos/19523383/thumb/3.jpg',
    date: new Date(2021, 1, 15, 19, 8),
    hitPercentage: 62.3,
    passed: false,
  },
  {
    sellerName: 'Raquel Shutz',
    sellerImage: 'https://qodebrisbane.com/wp-content/uploads/2019/07/This-is-not-a-person-10-1.jpeg',
    date: new Date(2021, 1, 6, 14, 32),
    hitPercentage: 27.8,
    passed: false,
  },
];

const ViewProductPage = () => {
  const dispatch = useDispatch();
  const history = useHistory();
  const location = useLocation();
  const toast = useRef<Toast>(null);

  const { loadingGetById, viewData, loadingDelete } = useSelector((state: StoreState) => state.products);

  const { id } = useParams<{id: string}>();

  useEffect(() => {
    if ((!id || id === ':id')) {
      goToList();
    }
    if (id) {
      dispatch(getById(id));
    }
  }, [id]);

  const currentRoute = allRoutes.find((r) => r.id === 'view-product');
  const editRoute = allRoutes.find((r) => r.id === 'edit-product');
  const listRoute = allRoutes.find((r) => r.id === 'productsGrouper')?.subPages.find((r) => r.id === 'product-list');

  const onEdit = () => {
    history.push(generatePath(editRoute?.path ?? '/', { id }));
  };

  const goToList = () => {
    history.push(listRoute?.path ?? '/');
  };

  const pageTitle = viewData?.name ? `Produto ${viewData?.name}` : currentRoute?.title ?? '';

  useEffect(() => {
    if (currentRoute) {
      dispatch(setCurrentPage({
        icon: currentRoute.icon,
        routeId: currentRoute.id,
        title: pageTitle,
        pageImage: currentRoute.pageImage,
      }));
      dispatch(pushBreadcrumb({
        path: location.pathname,
        routeId: currentRoute.id,
        title: pageTitle,
      }));
    }
  }, [currentRoute, viewData]);

  useEffect(() => {
    if ((!id || id === ':id')) {
      goToList();
    }
  }, [id]);

  const onDeleteSuccess = () => {
    toast.current?.show({
      severity: 'success', summary: 'Deletado!', detail: 'Registro removido com sucesso!', life: 1500,
    });
    setTimeout(goToList, 1500);
  };

  const confirmDelete = () => {
    confirmDialog({
      message: 'Deseja realmente deletar esse registro?',
      icon: 'pi pi-info-circle',
      acceptClassName: 'p-button-danger',
      // @ts-ignore
      header: 'Confirmar deleção',
      accept: () => dispatch(deleteRequest({ id, callBack: onDeleteSuccess })),
    });
  };

  return (
    <>
      <Toast ref={toast} />
      <Helmet>
        <title>{pageTitle}</title>
        <meta name="description" content="Visualizar uma produto" />
      </Helmet>
      <Zoom in>
        <div id="view-product-page" className="p-grid p-align-center">
          <Card className="p-col-12 p-xl-10" style={{ position: 'relative' }}>
            {(loadingGetById || loadingDelete) && (
              <>
                <Skeleton className="p-skeleton-loader" />
                <ProgressSpinner className="loader-app" />
              </>
            )}
            <div className="header-area">
              <img
                alt={viewData.name}
                src={viewData.mainImage}
                onError={(e: any) => {
                  e.target.src = 'data:image/png;base64,';
                }}
                style={{ verticalAlign: 'middle', borderRadius: '12px' }}
                className="p-shadow-3 avatar-image"
              />
              <Button
                label="Deletar"
                className="p-button-danger"
                type="button"
                icon="pi pi-trash"
                onClick={confirmDelete}
              />
              <Button
                label="Editar"
                type="button"
                icon="pi pi-pencil"
                onClick={onEdit}
              />
            </div>
            <Divider />
            <TabView className="tabview-custom">
              <TabPanel header="Informações Gerais" leftIcon="pi pi-desktop">
                <div className="p-grid">
                  <div className="p-col p-grid">
                    <div className="p-col-4">
                      <div className="label">Nome:</div>
                      <div className="label-value">{viewData.name}</div>
                    </div>
                    <div className="p-col-4">
                      <div className="label">Empresa:</div>
                      <div className="label-value">{viewData.companyName}</div>
                    </div>
                    <div className="p-col-4">
                      <div className="label">Categoria:</div>
                      <div className="label-value">{viewData.categoryName}</div>
                    </div>
                    <div className="p-col-12">
                      <div className="label">Descrição:</div>
                      <div className="label-value">{viewData.description}</div>
                    </div>
                    <div className="p-col-3">
                      <div className="label">Treinamentos Executados:</div>
                      <div className="label-value">{viewData.totalTreinamentDones}</div>
                    </div>
                    <div className="p-col-3">
                      <div className="label">Comprovações Enviadas:</div>
                      <div className="label-value">{viewData.totalProvesSent}</div>
                    </div>
                    <div className="p-col-3">
                      <div className="label">Data de Criação:</div>
                      <div className="label-value">{viewData.createdAt ? format(new Date(viewData.createdAt), 'dd/MM/yyyy') : ''}</div>
                    </div>
                    <div className="p-col-3">
                      <div className="label">Última Atualização:</div>
                      <div className="label-value">{viewData.updatedAt ? format(new Date(viewData.updatedAt), 'dd/MM/yyyy') : ''}</div>
                    </div>
                    <div className="p-col-8 p-offset-2">
                      <ImageGallery thumbnailPosition="left" showFullscreenButton={false} showPlayButton={false} items={viewData.images.map((img) => ({ original: img, thumbnail: img }))} />
                    </div>
                  </div>
                </div>
              </TabPanel>
              <TabPanel header="Provas Enviadas" leftIcon="pi pi-send">
                <ProductProvesSentView locations={mockLocations} />
              </TabPanel>
              <TabPanel header="Treinamentos Concluídos" leftIcon="pi pi-desktop">
                <ProductTreinamentView treinaments={mockTreinamentDones} />
              </TabPanel>
            </TabView>
          </Card>
        </div>
      </Zoom>
    </>
  );
};

export default ViewProductPage;
