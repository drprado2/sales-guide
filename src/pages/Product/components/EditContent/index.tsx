...
import {
  useDrag, DndProvider, useDrop, DropTargetMonitor,
} from 'react-dnd';
import update from 'immutability-helper';
import { HTML5Backend } from 'react-dnd-html5-backend';
import { StoreState } from '../../../../store';
import { setUpdateFormField } from '../../../../store/modules/products/slice';
...
const EditContent : React.FC<Props> = ({ images, productName }) => {
  ...
  const reOrderContentBlock = useCallback(
    (dragIndex: number, hoverIndex: number) => {
      const dragCard = updateForm.contents.value[dragIndex];

      const newContents = update<Array<Content>>(updateForm.contents.value, {
        $splice: [
          [dragIndex, 1],
          [hoverIndex, 0, dragCard],
        ],
      });
      dispatch(setUpdateFormField({ fieldName: 'contents', value: newContents }));
    },
    [updateForm],
  );
  ...
  const updateCurrentContent = () => {
    const newContets = updateForm.contents.value.map((c) => {
    ...
    dispatch(setUpdateFormField({ fieldName: 'contents', value: newContets }));
    toggleModal();
    resetCurrentForm();
  };
  ...
  const addCurrentContent = () => {
    const newContets = updateForm.contents.value.concat([{
      id: v4(),
      content: editorContent === '<p><br></p>' || editorContent === '<p></p>' ? '' : editorContent,
      title: currentForm.title,
      icon: currentForm.icon,
  ...
  const deleteContentItem = (id: string) => {
    dispatch(setUpdateFormField({ fieldName: 'contents', value: updateForm.contents.value.filter((c) => c.id !== id) }));
  };

  const editItem = (id: string) => {
    const item = updateForm.contents.value.find((c) => c.id === id);
    if (!item) return;
    setCurrentForm({
      id: item.id,
      title: item.title,
      ...
      <div className="mobile-phone">
        ...
        <div className="screen">
          ...
          <DndProvider backend={HTML5Backend}>
            {
              updateForm.contents.value.map((c, i) => (c.isCollapsable ? (
                <ContentItem
                  onEdit={editItem}
