...
interface Props {
  treinaments: Array<Treinament>;
}

const ProductTreinamentView : React.FC<Props> = ({ treinaments }) => {
  const classes = useStyles();
  const [localTreinaments, setLocalTreinaments] = useState(treinaments.map((t) => ({ ...t, hitPercentage: 0 })));

  useEffect(() => {
    setLocalTreinaments(treinaments);
  }, [treinaments]);

  return (
    <div className="seller-treinament-view">
      <List className={classes.root}>
        {
          localTreinaments.map((t) => (
            <>
              <ListItem key={t.sellerName}>
                <ListItemAvatar>
                  <img
                    alt={t.sellerName}
                    src={t.sellerImage}
                    ...
                    style={{
                      verticalAlign: 'middle', borderRadius: '12px', width: 80, height: 'auto', marginLeft: '-20px', marginRight: 10,
                    }}
                    className="p-shadow-3"
                  />
                </ListItemAvatar>
                <ListItemText primary={`Executado pelo vendedor ${t.sellerName}`} secondary={`Concluído ${format(t.date, "dd/MM/yyyy HH:mm'h'")}`} />
                <div style={{ display: 'flex', width: '40%', flexDirection: 'column' }}>
                  <h4>{t.passed ? 'Aprovado' : 'Reprovado'} com {t.hitPercentage}% de acerto</h4>
                  <ProgressBar color={t.passed ? 'green' : 'red'} style={{ width: '100%', height: 10, borderRadius: 10 }} value={t.hitPercentage} />
                </div>
              </ListItem>
              <Divider />
            </>
...
export default ProductTreinamentView;
