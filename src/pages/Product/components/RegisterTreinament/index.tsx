...
const RegisterTreinament : React.FC = () => {
  ...
  const deleteQuestion = (id: string) => {
    dispatch(setUpdateTreinamentFormField({ fieldName: 'questions', value: updateTreinamentForm.questions.value.filter((q) => q.id !== id) }));
    handleClose();
  };

  const editQuestion = (question: TreinamentQuestion) => {
    console.log('vindo editar', question);
    setShowModal(true);
    setCurrentForm({
      id: question.id,
  ...
  const itemTemplate = (question: TreinamentQuestion) => {
    console.log('passando na question', question);
    return (
      <div
  ...
  const addCurrentOption = () => {
    const questions = updateTreinamentForm.questions.value.concat([{
      id: v4(),
    ...
    dispatch(setUpdateTreinamentFormField({ fieldName: 'questions', value: questions }));
    setShowModal(false);
  };

  const updateCurrentOption = () => {
    const questions = updateTreinamentForm.questions.value.map((o) =>
      (o.id === currentForm.id ? ({
        id: o.id,
  ...
  return (
    <div className="register-treinament">
      ...
      <OrderList
        value={updateTreinamentForm.questions.value}
        header={(
      ...
      />
      <span className="p-error p-d-block">{updateTreinamentForm.questions.errors.join(', ')}</span>
    </div>
  );
};
