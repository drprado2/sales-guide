...
import { getList, setPaginateFilter, deleteRequest } from '../../../store/modules/products/slice';
import { StoreState } from '../../../store';
import EmptyTable from '../../../components/EmptyTable';
import { SellerList } from '../../../store/modules/sellers/types';
import { ProductList } from '../../../store/modules/products/types';
...
const ProductsPage = () => {
  ...
  const currentRoute = allRoutes.find((r) => r.id === 'productsGrouper')?.subPages.find((r) => r.id === 'product-list');
  const viewRoute = allRoutes.find((r) => r.id === 'view-product');
  const editRoute = allRoutes.find((r) => r.id === 'edit-product');
  const createRoute = allRoutes.find((r) => r.id === 'create-product');
  ...
  const mainImageTemplate = (rowData: ProductList) => (
    <>
      <img
        alt={rowData.name}
  ...
  return (
    <div id="products-page">
      ...
      <Helmet>
        ...
        <meta name="description" content="Visualizar a produto" />
      </Helmet>
      <Zoom in>
        <Card>
          ...
          >
            <Column body={mainImageTemplate} headerStyle={{ width: '6%', textAlign: 'center' }} bodyStyle={{ textAlign: 'center', overflow: 'visible' }} exportable={false} />
