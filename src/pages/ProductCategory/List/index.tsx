...
import { getList, setPaginateFilter, deleteRequest } from '../../../store/modules/productCategories/slice';
import { StoreState } from '../../../store';
import EmptyTable from '../../../components/EmptyTable';
import { SellerList } from '../../../store/modules/sellers/types';
import { ProductCategoryList } from '../../../store/modules/productCategories/types';
...
const ProductCategoriesPage = () => {
  ...
  const currentRoute = allRoutes.find((r) => r.id === 'productsGrouper')?.subPages.find((r) => r.id === 'productCategory-list');
  const viewRoute = allRoutes.find((r) => r.id === 'view-productCategory');
  const editRoute = allRoutes.find((r) => r.id === 'edit-productCategory');
  const createRoute = allRoutes.find((r) => r.id === 'create-productCategory');
  ...
  const iconTemplate = (rowData: ProductCategoryList) => (
    <>
      <img
        alt={rowData.name}
        src={rowData.icon}
  ...
  return (
    <div id="productCategories-page">
      ...
      <Helmet>
        ...
        <meta name="description" content="Visualizar a categoria de produto" />
      </Helmet>
      <Zoom in>
        <Card>
          ...
          >
            <Column body={iconTemplate} headerStyle={{ width: '5%', textAlign: 'center' }} bodyStyle={{ textAlign: 'center', overflow: 'visible' }} exportable={false} />
