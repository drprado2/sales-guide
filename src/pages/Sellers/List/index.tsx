...
import {
  getList, setPaginateFilter, deleteRequest, setFilter,
...
const SellersPage = () => {
  ...
  const {
    paginated, loadingGetList, paginateFilter, loadingDelete, filter,
  ...
  const currentRoute = allRoutes.find((r) => r.id === 'seller-list');
  const viewRoute = allRoutes.find((r) => r.id === 'view-seller');
  const editRoute = allRoutes.find((r) => r.id === 'edit-seller');
  const createRoute = allRoutes.find((r) => r.id === 'create-seller');
  ...
  const enableTemplate = (rowData: SellerList) => (
    <Tag severity={rowData.enable ? 'success' : 'danger'} value={rowData.enable ? 'Ativo' : 'Inativo'} icon={rowData.enable ? 'pi pi-check-circle' : 'pi pi-exclamation-triangle'} />
  );

  const avatarTemplate = (rowData: SellerList) => (
    <>
      <img
        alt={rowData.name}
        src={rowData.avatarImage}
        onError={(e: any) => {
  ...
  return (
    <div id="sellers-page">
      ...
      <Helmet>
        ...
        <meta name="description" content="Listagem de vendedores" />
      </Helmet>
      <Zoom in>
        <Card>
          ...
          >
            <Column body={avatarTemplate} headerStyle={{ width: '6%', textAlign: 'center' }} bodyStyle={{ textAlign: 'center', overflow: 'visible' }} exportable={false} />
            <Column field="name" header="Nome" sortable filter filterMatchMode="contains" filterPlaceholder="Pesquisar..." style={{ width: '15%', textOverflow: 'ellipsis', overflow: 'hidden' }} />
            <Column field="email" header="E-mail" sortable filter filterMatchMode="contains" filterPlaceholder="Pesquisar..." style={{ width: '19%', textOverflow: 'ellipsis', overflow: 'hidden' }} />
            <Column field="document" header="Documento" sortable filter filterMatchMode="contains" filterPlaceholder="Pesquisar..." style={{ width: '18%', textOverflow: 'ellipsis', overflow: 'hidden' }} />
            <Column field="enable" header="Ativo" body={enableTemplate} sortable filter filterElement={statusFilter} style={{ width: '13%' }} />
            <Column field="lastAccess" header="Último Acesso" sortable filter filterMatchMode="contains" filterPlaceholder="Pesquisar..." style={{ width: '14%', textOverflow: 'ellipsis' }} />
            <Column body={actionBodyTemplate} headerStyle={{ width: '58px', textAlign: 'end' }} bodyStyle={{ textAlign: 'center', overflow: 'visible' }} />
          </DataTable>
          <Paginator
