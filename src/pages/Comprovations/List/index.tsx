import React, {
  ChangeEvent, useEffect, useRef, useState,
} from 'react';
import './styles.scss';
import { useDispatch, useSelector } from 'react-redux';
import { DataTable } from 'primereact/datatable';
import { Column } from 'primereact/column';
import { Card } from 'primereact/card';
import { Button } from 'primereact/button';
import { Helmet } from 'react-helmet';
import {
  Zoom,
} from '@material-ui/core';
import { Skeleton } from 'primereact/skeleton';
import { ProgressSpinner } from 'primereact/progressspinner';
import { Paginator } from 'primereact/paginator';
import { InputText } from 'primereact/inputtext';
import { Toast } from 'primereact/toast';
import { useHistory } from 'react-router-dom';
import { Accordion, AccordionTab } from 'primereact/accordion';
import Tooltip from '@material-ui/core/Tooltip';
import { format } from 'date-fns';
import ImageGallery from 'react-image-gallery';
import { allRoutes } from '../../../store/modules/auth/slice';
import { resetBreadcrumbTo, setCurrentPage } from '../../../store/modules/template/slice';
import {
  getList, setPaginateFilter, setFilter,
} from '../../../store/modules/comprovations/slice';
import { StoreState } from '../../../store';
import { ComprovationList } from '../../../store/modules/comprovations/types';
import EmptyTable from '../../../components/EmptyTable';

const ComprovationsPage = () => {
  const dispatch = useDispatch();
  const {
    paginated, loadingGetList, paginateFilter, filter,
  } = useSelector((state: StoreState) => state.comprovations);
  const [globalFilter, setGlobalFilter] = useState<string | null>(null);
  const [expandedRows, setExpandedRows] = useState<any[]>([]);
  const dataTableRef = useRef<DataTable>(null);
  const [menuRef, setMenuRef] = React.useState<any>(null);
  const toast = useRef<Toast>(null);
  const history = useHistory();

  const currentRoute = allRoutes.find((r) => r.id === 'productsGrouper')?.subPages.find((r) => r.id === 'comprovation-list');

  useEffect(() => {
    dispatch(getList());
  }, []);

  useEffect(() => {
    if (currentRoute) {
      dispatch(setCurrentPage({
        icon: currentRoute.icon,
        routeId: currentRoute.id,
        title: currentRoute.title,
        pageImage: currentRoute.pageImage,
      }));
      dispatch(resetBreadcrumbTo({
        path: currentRoute.path,
        routeId: currentRoute.id,
        title: currentRoute.title,
      }));
    }
  }, [currentRoute]);

  const handleClose = () => {
    setMenuRef(null);
  };

  const renderHeader = () => (
    <div className="table-header">
      <Button
        label="Exportar CSV"
        className="p-button-success"
        type="button"
        icon="pi pi-file-excel"
        onClick={() => dataTableRef?.current?.exportCSV()}
      />
      <span className="p-input-icon-left">
        <i className="pi pi-search" />
        <InputText type="search" onInput={(e: ChangeEvent<HTMLInputElement>) => setGlobalFilter(e.target.value)} placeholder="Pesquisar em tudo" />
      </span>
    </div>
  );

  const productsTemplate = (rowData: ComprovationList) => (
    <div>{rowData.products.reduce((acc, curr, index) => `${acc + curr.name}${index === rowData.products.length - 1 ? '' : ', '}`, '')}</div>
  );

  const sellerImageTemplate = (rowData: ComprovationList) => (
    <>
      <img
        alt={rowData.sellerName}
        src={rowData.sellerImage}
        onError={(e: any) => {
          e.target.src = 'data:image/png;base64,';
        }}
        width="60"
        style={{ verticalAlign: 'middle', borderRadius: '10px' }}
      />
    </>
  );

  const rowExpansionTemplate = (data: ComprovationList) => (
    <div className="comprovation-area">
      <div>
        <h3>Imagens</h3>
        <ImageGallery showPlayButton={false} items={data.proveImages.map((img) => ({ original: img, thumbnail: img }))} />
        <h3>Descrição</h3>
        <p>{data.description}</p>
        <h3>Produtos</h3>
        <div className="p-grid">
          {
                    data.products.map((p) => (
                      <div key={p.name} className="p-col-3" style={{ position: 'relative' }}>
                        <img
                          alt={p.name}
                          src={p.image}
                          onError={(e: any) => {
                            e.target.src = 'data:image/png;base64,';
                          }}
                          style={{
                            verticalAlign: 'middle', borderRadius: '12px', width: '100%', height: 'auto',
                          }}
                          className="p-shadow-3"
                        />
                        <span className="img-alt">{p.name}</span>
                      </div>
                    ))
                  }
        </div>
      </div>
    </div>
  );

  const header = renderHeader();

  return (
    <div id="comprovations-page">
      <Toast ref={toast} />
      <Helmet>
        <title>{currentRoute?.title}</title>
        <meta name="description" content="Listagem de comprovações" />
      </Helmet>
      <Zoom in>
        <Card>
          {(loadingGetList) && (
            <>
              <Skeleton className="p-skeleton-loader" />
              <ProgressSpinner className="loader-app" />
            </>
          )}
          <DataTable
            ref={dataTableRef}
            header={header}
            globalFilter={globalFilter}
            value={paginated.data}
            dataKey="id"
            rowHover
            emptyMessage={<EmptyTable />}
            sortMode="multiple"
            removableSort
            exportFilename="comprovationos"
            expandedRows={expandedRows}
            onRowToggle={(e) => setExpandedRows(e.data)}
            rowExpansionTemplate={rowExpansionTemplate}
          >
            <Column expander style={{ width: '2em' }} />
            <Column body={sellerImageTemplate} headerStyle={{ width: '4%', textAlign: 'center' }} bodyStyle={{ textAlign: 'center', overflow: 'visible' }} exportable={false} />
            <Column field="sellerName" header="Vendedor" sortable filter filterMatchMode="contains" filterPlaceholder="Pesquisar..." style={{ width: '12%', textOverflow: 'ellipsis', overflow: 'hidden' }} />
            <Column field="clientName" header="Cliente" sortable filter filterMatchMode="contains" filterPlaceholder="Pesquisar..." style={{ width: '12%', textOverflow: 'ellipsis', overflow: 'hidden' }} />
            <Column
              field="productNames"
              header="Produtos"
              body={productsTemplate}
              sortable
              filter
              filterMatchMode="contains"
              filterField="products"
              filterFunction={((value, filter1) => {
                console.log(value, filter1);
              })}
              filterPlaceholder="Pesquisar..."
              style={{ width: '15%' }}
            />
            <Column field="date" header="Data" sortable filter filterMatchMode="contains" filterPlaceholder="Pesquisar..." style={{ width: '10%', textOverflow: 'ellipsis' }} />
            <Column field="description" header="Descrição" sortable filter filterMatchMode="contains" filterPlaceholder="Pesquisar..." style={{ width: '18%', textOverflow: 'ellipsis', overflow: 'hidden' }} />
          </DataTable>
          <Paginator
            rows={paginateFilter.total}
            first={paginateFilter.total * paginateFilter.current}
            currentPageReportTemplate="Mostrando {first} até {last} de {totalRecords}"
            template="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
            totalRecords={paginated.total}
            alwaysShow
            rowsPerPageOptions={[10, 25, 50, 100]}
            onPageChange={(e) => dispatch(setPaginateFilter({ current: e.first, total: e.rows }))}
          />
        </Card>
      </Zoom>
    </div>
  );
};

export default ComprovationsPage;
