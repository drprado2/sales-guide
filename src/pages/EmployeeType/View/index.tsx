...
const ViewEmployeeTypePage = () => {
  const dispatch = useDispatch();
  const history = useHistory();
  const location = useLocation();
  const toast = useRef<Toast>(null);

  const { loadingGetById, viewData, loadingDelete } = useSelector((state: StoreState) => state.employeeTypes);
  ...
  const currentRoute = allRoutes.find((r) => r.id === 'view-employeeType');
  ...
  const pageTitle = viewData?.name ? `Tipo de contratação ${viewData?.name}` : currentRoute?.title ?? '';
  ...
  return (
    <>
      ...
      <Helmet>
        ...
        <meta name="description" content="Visualizar um tipo de contratação" />
      </Helmet>
      <Zoom in>
        <div id="view-employeeType-page" className="p-grid p-align-center">
...
export default ViewEmployeeTypePage;
